...
TQ_detection <- TQ_detection %>%
  ...
  mutate(chnm = factor(chnm, levels=levels(chem_detection$chnm)), 
         group = factor(group, levels=rev(c("AboveTQ1", "AboveTQ0.1", "AboveTQ0.01", "BelowTQ0.01", "OnlyDetected"))))
...
chemicalbyEAR3 <- ggplot(data=chem_detection, aes(x=chnm, y=value, fill=group)) + 
  geom_bar(color = 'grey', width=.8, size=.1, stat='identity') +  
  # coord_flip() +
  facet_grid(.~Class, space="free", scales="free") +
...
chemicalbyEAR3_horiztonal <- ggplot(data=chem_detection, aes(x=chnm2, y=value, fill=group)) + 
  geom_bar(color = 'grey', width=.8, size=.1, stat='identity') +  
  coord_flip() +
  facet_grid(Class~., space="free", scales="free") +
  labs(x='Chemical', y='Number of rivers', fill = 'EAR') + 
...
print(chemicalbyEAR3_horiztonal)

ggsave(file_out(file.path(path_to_data, "Figures/StackBar_ByEAR_Bychem3_horiztonal.png")), plot = chemicalbyEAR3_horiztonal, height=7, width=5)
...
TQ_detection3 <- ggplot(data=TQ_detection , aes(x=chnm, y=value, fill=group)) +
  geom_bar(color = 'grey', width=.8, size=.1, stat='identity') +
  # coord_flip() +
  facet_grid(.~Class, space="free", scales="free") +
...
ggsave(file_out(file.path(path_to_data, "Figures/StackBar_ByTQ_Bychem3.png")), plot = TQ_detection3, height=5, width=6)
...
TQ_detection3_horiztonal <- ggplot(data=TQ_detection, aes(x=chnm2, y=value, fill=group)) + 
  geom_bar(color = 'grey', width=.8, size=.1, stat='identity') +  
  coord_flip() +
  facet_grid(Class~., space="free", scales="free") +
  labs(x='Chemical', y='Number of rivers', fill = 'TQ') + 
...
print(TQ_detection3_horiztonal)

ggsave(file_out(file.path(path_to_data, "Figures/StackBar_ByTQ_Bychem3_horiztonal.png")), plot = TQ_detection3_horiztonal, height=7, width=5)

#Combine EAR and TQ horizontal plots

EAR_box1 <- chemicalbyEAR3_horiztonal +
...
site_freq_EARAbove0<-chemicalSummary2 %>%
  group_by(site, CAS) %>%
  filter(EAR < 0.0001) %>%
  summarize(EAR_max = max(EAR, na.rm=T)) %>%
  tally(name = "BelowEAR0.0001")
...
site_detection <- site_detection %>%
  arrange(Lake, desc(AboveEAR0.01), desc(AboveEAR0.001), desc(AboveEAR0.0001), desc(BelowEAR0.0001), desc(OnlyDetected)) %>%
  tidyr::gather(key=group, value=value, -site, -shortName, -Lake) %>%
  mutate(group = factor(group, levels=rev(c("AboveEAR0.01", "AboveEAR0.001", "AboveEAR0.0001", "BelowEAR0.0001", "OnlyDetected"))))
...
sitebyEAR3 <- ggplot(data=site_detection, aes(x=shortName, y=value, fill=group)) +
  geom_bar(color = 'grey', width=.8, size=.1, stat='identity') +
  # coord_flip() +
  facet_grid(.~Lake, space="free", scales="free") +
  labs(x='Stream', y='Number of chemicals', fill = 'EAR') +
  ...
  scale_fill_manual(values = colors_EAR2, labels = c("Unknown EAR", expression(paste("EAR < 10"^"-4")),expression(paste("EAR > 10"^"-4")), expression(paste("EAR > 10"^"-3")), expression(paste("EAR > 10"^"-2")))) +
  # scale_fill_brewer(palette = "YlOrRd", labels = c("Detected", expression(paste("EAR > 10"^"-4")), expression(paste("EAR > 10"^"-3")))) +
  scale_y_continuous(limits=c(0,48.5), expand=c(0,0)) +
  theme(legend.position = 'bottom') +
  guides(fill = guide_legend(title.position='left', title.hjust=0.5, reverse=T)) +
  theme(axis.text.x = element_text(angle=90, vjust=0.5, hjust=1))
...
ggsave(file_out(file.path(path_to_data, "Figures/StackBox_ByEAR_BySite3.png")), plot = sitebyEAR3, height=4, width=6)
