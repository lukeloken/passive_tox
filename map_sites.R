...
chemicalSummary_conc <- make('chemicalSummary_conc')
chemicalSummary <- make('chemicalSummary')
threshold <- 10^-3

nchems <- chemicalSummary_conc %>%
  group_by(site) %>%
  summarize(nUniqueChems = length(unique(chnm)))
...
sites <- left_join(sites, graphData, by = c('site_no' = 'site')) %>%
  left_join(metolachlor, by = c('site_no' = 'site')) %>%
  left_join(fipronil, by = c('site_no' = 'site')) %>%
  left_join(NR1I2,by = c('site_no' = 'site')) %>%
  left_join(PTGS2, by = c('site_no' = 'site')) %>%
  left_join(TP53, by = c('site_no' = 'site'))
  
  
sites_df <- st_as_sf(sites[, c("dec_long_va","dec_lat_va", 'nChem', 'count', 'nUniqueChems', 'maxsumEAR_metolachlor', 'maxsumEAR_fipronil',
                               'NR_top_chem', 'NR_maxsumEAR', 'NR_max_nchems',
                               'PT_top_chem', 'PT_maxsumEAR','PT_max_nchems',
                               'TP_top_chem', 'TP_maxsumEAR', 'TP_max_nchems')],
...
first_pass <- ggplot() + 
  ...
  geom_sf(data = sites_proj, alpha = 0.9, shape = 16, aes(size = nUniqueChems, color = nChem), show.legend = FALSE) +

  scale_size(range = c(2,14), breaks = c(5,25,45,65), guide = 'legend') +
